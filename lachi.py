    ...
    if is_admin(message.author.id) and bot.user in message.mentions:
            ...
            if len(parts) >= 2:
                ...
                if user:
                    if user.id == config.master_id:
                        ...
                        await dm_channel.send("  Master already has full access")
                    elif user.id in admin_users:
                        await message.add_reaction(' ')
                        await dm_channel.send(f"  {user.name} is already an admin")
                    else:
        ...
        if message.author.id == config.master_id and content.startswith('removeadmin '):
            ...
            if len(parts) >= 2:
                ...
                if user:
                    if user.id == config.master_id:
                        await message.add_reaction(' ')
                        await dm_channel.send("  Cannot remove master")
                    elif user.id in admin_users:
                        ...
                        await message.add_reaction(' ')
                        await dm_channel.send(f"  {user.name} is not an admin")
                else:
                    await message.add_reaction(' ')
                    await dm_channel.send(f"  User not found: {parts[1]}")
            return
            ...
            if len(parts) >= 3:
                ...
                if user:
                    ...
                    if user.id == config.master_id and message.author.id != config.master_id:
                        await message.add_reaction(' ')
                        await dm_channel.send("  Admins cannot add reactions to Master")
                        return
            ...
            if len(parts) >= 2:
                ...
                if user:
                    if user.id in targets:
                        ...
                        await message.add_reaction(' ')
                        await dm_channel.send(f"  Not reacting to {user.name}")
                else:
                    await message.add_reaction(' ')
                    await dm_channel.send(f"  User not found: {parts[1]}")
            return
        ...
        if content == 'list':
            if targets:
                ...
                await message.add_reaction(' ')
                await dm_channel.send("  No active targets")
            return
